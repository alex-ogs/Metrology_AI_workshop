...
from sfs_sensor.model_architecture import model_architectures as ma
...
def run_deep_training(
    ...
    if use_parameter_server and (worker_replicas > 0): # Environment variable to select if ps strategy is applied
        ...
        ds_train = ds_train.shuffle(buffer_size=min(5000, len(X_train))).batch(batch_size).repeat().prefetch(1)
...
def run_image_training(
    ...
    if use_parameter_server and (worker_replicas > 0):
        ...
        with strategy.scope():
            model = ma.build_sfs_cnn_model(input_shape, num_outputs=2, flat=flat_layer)
            optimizer = tf.keras.optimizers.Adam(learning_rate=1e-4)
...
def parse_args(argv: List[str]):
    ...
    parser.add_argument("--data-path", default=os.environ.get("DATA_PATH", "/app/sfs_sensor/dataset_acquisition/pointing-laser"), help="Path to CSV or image root directory")
    ...
    parser.add_argument("--img-height", type=int, default=int(os.environ.get("IMG_HEIGHT", "1024")), help="Image height for resizing")
    parser.add_argument("--img-width", type=int, default=int(os.environ.get("IMG_WIDTH", "1280")), help="Image width for resizing")
